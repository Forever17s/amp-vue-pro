<template>
  <span
    ...
    :class="[
      ...
      cascaderSize ? 'py-cascader--' + cascaderSize : ''
    ]"
    @click="handleClick"
    ...
    >
      <template slot="suffix">
        <i
          ...
          class="py-input__icon py-icon-circle-close py-cascader__clearIcon"
          @click="clearValue"
        ></i>
        <i
          key="2"
          v-else
          class="py-input__icon py-icon-arrow-down"
          :class="{ 'is-reverse': menuVisible }"
        ></i>
      </template>
    </py-input>
    <span class="py-cascader__label" v-show="inputValue === ''">
...
<script>
import Vue from 'vue';
import PyCascaderMenu from './menu';
import PyInput from '../../input';
import Popper from 'src/common/utils/vue-popper';
...
export default {
  ...
  methods: {
    initMenu() {
      this.menu = new Vue(PyCascaderMenu).$mount();
    ...
    handleKeydown(e) {
      ...
      if (keyCode === 13) {
        ...
        setTimeout(() => {
          const firstMenu = this.popperElm.querySelectorAll('.py-cascader-menu')[0];
    ...
    highlightKeyword(label, keyword) {
      ...
      return label.split(keyword)
        .map((node, index) => index === 0 ? node : [
          h('span', { class: { 'py-cascader-menu__item__keyword': true }}, [this._v(keyword)]),
          node
        ]);
    },
