<script>
  import { isDef } from 'src/common/utils/shared';
  import scrollIntoView from 'src/common/utils/scroll-into-view';
    ...
    render(h) {
      ...
      const menus = this._l(activeOptions, (menu, menuIndex) => {
        ...
        const items = this._l(menu, item => {
          ...
          if (!item.disabled) {
            ...
            events.on.keydown = (ev) => {
              ...
              const currentPye = ev.target;
              const parentPye = this.$refs.menus[menuIndex];
              const menuItemList = parentPye.querySelectorAll("[tabindex='-1']");
              const currentIndex = Array.prototype.indexOf.call(menuItemList, currentPye); // 当前索引
              ...
              if ([38, 40].indexOf(keyCode) > -1) {
                ...
                if (!item.children) {
                  const id = currentPye.getAttribute('id');
                  parentPye.setAttribute('aria-activedescendant', id);
          ...
          return (
            <li
              class={{
                'py-cascader-menu__item': true,
                'py-cascader-menu__item--extensible': item.children,
        ...
        return (
          <ul
            class={{
              'py-cascader-menu': true,
              'py-cascader-menu--flexible': isFlat
            }}
            {...hoverMenuEvent}
            style={menuStyle}
            refInFor
      ...
      return (
        <transition name="py-zoom-in-top" on-before-enter={this.handleMenuEnter} on-after-leave={this.handleMenuLeave}>
          <div
            v-show={visible}
            class={[
              'py-cascader-menus py-popper',
              popperClass
            ]}
            ref="wrapper"
          >
