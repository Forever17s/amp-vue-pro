<template>
  <py-input
    class="py-date-editor"
    :class="'py-date-editor--' + type"
    ...
    ref="reference">
    <i slot="suffix"
      class="py-input__icon"
      @click="handleClickIcon"
      :class="{ 'py-icon-circle-close': showClose }"
      v-if="haveTrigger">
    </i>
  </py-input>
  <div
    class="py-date-editor py-range-editor py-input__inner"
    :class="[
      'py-date-editor--' + type,
      pickerSize ? `py-range-editor--${ pickerSize }` : '',
    ...
    v-clickoutside="handleClose"
    v-else>
    <i :class="['py-input__icon', 'py-range__icon', triggerClass]"></i>
    <input
      ...
      @focus="handleFocus"
      class="py-range-input">
    <span class="py-range-separator">{{ rangeSeparator }}</span>
    <input
    ...
    <i
      ...
      v-if="haveTrigger"
      :class="{ 'py-icon-circle-close': showClose }"
      class="py-input__icon py-range__close-icon">
    </i>
  </div>
</template>
...
export default {
  ...
  watch: {
    pickerVisible(val) {
      ...
      if (val) {
        ...
        this.dispatch('PyFormItem', 'el.form.blur');
        this.blur();
      }
    },
    parsedValue: {
  ...
  computed: {
    ...
    triggerClass() {
      return this.type.indexOf('time') !== -1 ? 'py-icon-time' : 'py-icon-date';
    },
