...
const getI18nSettings = () => {
  return {
    dayNamesShort: weeks.map(week => t(`py.datepicker.weeks.${ week }`)),
    dayNames: weeks.map(week => t(`py.datepicker.weeks.${ week }`)),
    monthNamesShort: months.map(month => t(`py.datepicker.months.${ month }`)),
    monthNames: months.map((month, index) => t(`py.datepicker.month${ index + 1 }`)),
    amPm: ['am', 'pm']
  };
};
