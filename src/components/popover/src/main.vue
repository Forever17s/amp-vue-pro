<template>
  <span>
    <transition :name="transition" @after-leave="doDestroy">
      <div
        class="py-popover py-popper"
        :class="[popperClass, content && 'py-popover--plain']"
        ref="popper"
      ...
      >
        <div class="py-popover__title" v-if="title" v-text="title"></div>
        <slot>{{ content }}</slot>
      </div>
    </transition>
...
import { on, off } from 'src/common/utils/dom';
...
export default {
  ...
  mounted() {
    ...
    // 可访问性
    if (reference) {
      addClass(reference, 'py-popover__reference');
