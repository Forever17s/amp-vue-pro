<template>
    <div>
        <div v-if="tableSearch">
            <Py-col :span="6" style="float:right;margin-bottom: 10px;max-width: 200px">
    ...
    <div class="py-table__column-resize-proxy" ref="resizeProxy" v-show="resizeProxyVisible"></div>
        ...
        <div v-if="tablePagination" style="overflow: hidden">
            <Py-col  :span="24"v-show="this.total > 10">
                <PyPagination layout="total, sizes, prev, pager, next, jumper"
...
import { addResizeListener, removeResizeListener } from 'src/common/utils/resize-event';
...
import PyPagination from '../../pagination';
  ...
  export default {
    ...
    methods: {
      ...
      doLayout() {
        ...
        resetData(){
          this.originData = this.myData;
          this.searchTableContent()
        },

        searchTableContent(){
        // save origin data
          // 获取当前表格表头信息，只对表格展示信息进行筛选
          const tableColumns = this.store.states.columns;
          const searchKey = this.trim(this.searchKey).toLowerCase() ;
          this.searchData = this.originData.filter((item) => {
            if(!searchKey) return true;
            else{
              for(let i = 0; i < tableColumns.length; i++){
                const colProp = tableColumns[i].property;
                if(item[colProp] && item[colProp].toString().toLowerCase().indexOf(searchKey) !== -1) return true;
              }
            }
          });
          this.total = this.searchData.length;
          !this.tablePagination ? this.searchData : this.searchData = this.searchData.filter((data, index) => index < this.pageSize * this.page && index >= this.pageSize * (this.page - 1));
          this.transData = this.searchData;
        },
